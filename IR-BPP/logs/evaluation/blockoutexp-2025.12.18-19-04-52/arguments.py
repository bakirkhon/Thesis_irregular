import argparse  # allows to read command-line arguments
import torch
import numpy as np
...
def get_args():
    # Note that hyperparameters may originally be reported
    # in ATARI game frames instead of agent steps
    parser = argparse.ArgumentParser(description='Rainbow for IR BPP')

    # ===== Parameters for the reinforcement learning agent =====
    parser.add_argument('--hidden-size', type=int, default=128, metavar='SIZE',
                        help='Network hidden size')
    parser.add_argument('--noisy-std', type=float, default=0.5, metavar='σ',
                        help='Initial standard deviation of noisy linear layers')
    parser.add_argument('--atoms', type=int, default=31, metavar='C',
                        help='Discretised size of value distribution')
    parser.add_argument('--V-min', type=float, default=-1, metavar='V',
                        help='Minimum of value distribution support')
    parser.add_argument('--V-max', type=float, default=8, metavar='V',
    ...
    parser.add_argument('--architecture', type=str, default='canonical',
                        choices=['canonical', 'dataset-efficient'], metavar='ARCH',
    ...
    # TODO: DeepMind evaluation uses 500K frames every 1M steps
    ...
    parser.add_argument('--envName', type=str, default='Physics-v0',
                        help='The environment name for packing policy training and testing')
    parser.add_argument('--dataset', type=str, default='blockout',
                        help='The organized dataset folder for training')
    parser.add_argument('--device', type=int, default=0,
                        help='GPU device ID')
    parser.add_argument('--custom', type=str, default=None,
                        help='Customized label for the experiment')
    parser.add_argument('--hierachical', action='store_true',
                        help='Use hierachical policy')
    parser.add_argument('--bufferSize', type=int, default=1,
                        help='Object buffer size')
    parser.add_argument('--num_processes', type=int, default=2,
                        help='How many parallel processes used for training')
    parser.add_argument('--distributed', action='store_true',
                        help='Use distributed training')
    parser.add_argument('--samplePointsNum', type=int, default=1024,
                        help='How many points to sample from the object surface')
    parser.add_argument('--selectedAction', type=int, default=500,
                        help='How many actions to select from the action space')
    parser.add_argument('--maxBatch', type=int, default=2,
                        help='How many batches for simulation')
    parser.add_argument('--visual', action='store_true',
                        help='Render the scene')
    parser.add_argument('--resolutionA', type=float, default=0.02,
                        help='The resolution for the action space')
    parser.add_argument('--resolutionH', type=float, default=0.01,
                        help='The resolution for the heightmap')
    parser.add_argument('--resolutionZ', type=float, default=0.01,
                        help='The resolution for the z axis')
    parser.add_argument('--resolutionRot', type=int, default=8,
                        help='The resolution for the rotation')
    parser.add_argument('--locmodel', type=str, default=None,
                        help='Path to trained location model')
    parser.add_argument('--ordmodel', type=str, default=None,
                        help='Path to trained order model')
    parser.add_argument('--only-simulate-current', action='store_true',
                        help='Only simulate the current item')
    parser.add_argument('--non_blocking', action='store_true',
                        help='Train actor and critic in non-blocking mode')
    parser.add_argument('--time_limit', type=float, default=0.01,
                        help='Time limit for each simulation step')
    ...
    args.bin_dimension = np.round([0.32, 0.32, 0.2], decimals=6)
    args.ZRotNum = args.resolutionRot
    args.objVecLen = 9
    ...
    args.shapeDict, args.infoDict = load_shape_dict(
        args,
        True,
        scale=args.meshScale
    )
    ...
    if args.evaluate:
        args.num_processes = 1  # temp setting
