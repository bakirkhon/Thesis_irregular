...
class Space(object):
    ...
    def get_possible_position(self, next_item_ID, next_item, selectedAction):
        ...
        for rotIdx in range(rotNum):
            ...
            boundingSize = np.array(next_item[rotIdx].extents, dtype=np.float64)

            # IMPORTANT: avoid "ceil" jumping by 1 because of tiny floating error
            epsH = 1e-9
            epsA = 1e-9

            rangeX_OH, rangeY_OH = np.ceil((boundingSize[0:2] - epsH) / self.resolutionH).astype(np.int32)
            rangeX_OA, rangeY_OA = np.ceil((boundingSize[0:2] - epsA) / self.resolutionAct).astype(np.int32)

            # Clamp to bin grid sizes (prevents 29 in a 28 grid from killing all placements)
            rangeX_OH = int(np.clip(rangeX_OH, 1, self.rangeX_C))
            rangeY_OH = int(np.clip(rangeY_OH, 1, self.rangeY_C))
            rangeX_OA = int(np.clip(rangeX_OA, 1, self.rangeX_A))
            rangeY_OA = int(np.clip(rangeY_OA, 1, self.rangeY_A))
            ...
            for X in range(self.rangeX_A - rangeX_OA + 1):
                for Y in range(self.rangeY_A - rangeY_OA + 1):
                    ...
                    # --- slice heightmap ---
                    h_slice = self.heightmapC[
                        coorX : coorX + rangeX_OH,
                        coorY : coorY + rangeY_OH
                    ]

                    # --- get actual slice size ---
                    sx, sy = h_slice.shape

                    # --- crop object maps to match ---
                    hB = heightMapB[:sx, :sy]
                    mB = maskB[:sx, :sy]

                    # --- compute placement height ---
                    posZ = np.max((h_slice - hB) * mB)
