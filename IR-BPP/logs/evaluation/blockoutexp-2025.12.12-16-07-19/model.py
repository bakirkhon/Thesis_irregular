...
class DQNBPP(nn.Module):
  def __init__(self, args, action_space, shapeArray):
    ...
    self.MapLength = int(env.bin_sequence[env.current_bin_index][0] / self.resolutionH)
  ...
  def embed_heightmap_and_sampled_point_cloud(self, x):
      ...
      self.MapLength = int(env.bin_sequence[self.current_bin_index][0] / self.resolutionH)
    #   print("current_index:",env.current_bin_index)
      try:
        # print("env.bin_sequence[env.current_bin_index][0]:",env.bin_sequence[env.current_bin_index][0])
        heightMap = heightMap.reshape((batchSize, 1, self.MapLength, self.MapLength))
      except:
        self.current_bin_index+=1
        self.MapLength = int(env.bin_sequence[self.current_bin_index][0] / self.resolutionH)
        print("env.bin_sequence[env.current_bin_index][0]:",env.bin_sequence[self.current_bin_index][0])
