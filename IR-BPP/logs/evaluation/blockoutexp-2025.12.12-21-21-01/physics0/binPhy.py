...
class PackingGame(gym.Env):
    def __init__(self,
        ...
        self.bin_sequence = [
            ...
            np.round([0.32, 0.32, 0.18], decimals=6),
            np.round([0.24, 0.24, 0.18], decimals=6)
        ]        
        self.current_bin_index = 0
        self.bin_results = []
        self.shotInfo = args['shotInfo']
        self.fixed_heightmap_size = 32  # Always 32×32 grid
        self.resolutionH = self.calculate_resolution(self.bin_sequence[0])
    ...
    def calculate_resolution(self, bin_dims):
        """
        Calculate resolution to keep heightmap at fixed size.
        
        For a bin of size W×H with fixed grid size N×N:
        resolution = W / N = H / N
        
        Examples:
        - 0.34m bin with 32×32 grid: resolution = 0.34/32 ≈ 0.0106
        - 0.26m bin with 32×32 grid: resolution = 0.26/32 ≈ 0.0081
        """
        resolution = bin_dims[0] / self.fixed_heightmap_size
        return np.round(resolution, decimals=6)
    ...
    def next_bin(self):
        ...
        if self.current_bin_index < self.total_bins and self.total_items < self.item_limit:
            try:    
                ...
                self.resolutionH = self.calculate_resolution(next_bin_size)
    ...
    def reset(self, index=None):
        ...
        self.resolutionH = self.calculate_resolution(current_bin_size)
    ...
    def cur_observation(self, genItem = True, draw = False):
        ...
        if not self.chooseItem:
            ...
            if self.heightMapPre:
                print("not reached the point 691")
