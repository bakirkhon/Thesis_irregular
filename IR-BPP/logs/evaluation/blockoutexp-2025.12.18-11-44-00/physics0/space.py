...
class Space(object):
    ...
    #     return naiveMask
    # Replace the get_possible_position method in space.py with this version:
        ...
        for rotIdx in range(rotNum):
            ...
            for X in range(self.rangeX_A - rangeX_OA + 1):
                for Y in range(self.rangeY_A - rangeY_OA + 1):
                    ...
                    # Get the heightmap slice
                    heightmap_slice = self.heightmapC[coorX:coorX + rangeX_OH, coorY:coorY + rangeY_OH]
                    
                    # Check if shapes match
                    if heightmap_slice.shape != heightMapB.shape:
                        # Pad heightmap_slice if it's smaller
                        if heightmap_slice.shape[0] < heightMapB.shape[0] or heightmap_slice.shape[1] < heightMapB.shape[1]:
                            padded_slice = np.zeros_like(heightMapB)
                            padded_slice[:heightmap_slice.shape[0], :heightmap_slice.shape[1]] = heightmap_slice
                            heightmap_slice = padded_slice
                        # Crop heightMapB and maskB if heightmap_slice is larger (shouldn't happen)
                        else:
                            heightMapB_crop = heightMapB[:heightmap_slice.shape[0], :heightmap_slice.shape[1]]
                            maskB_crop = maskB[:heightmap_slice.shape[0], :heightmap_slice.shape[1]]
                            posZ = np.max((heightmap_slice - heightMapB_crop) * maskB_crop)
                            if np.round(posZ + boundingSize[2] - self.bin_dimension[2], decimals=6) <= 0:
                                naiveMask[rotIdx, X, Y] = 1
                            self.posZmap[rotIdx, X, Y] = posZ
                            continue
                    ...
                    posZ = np.max((heightmap_slice - heightMapB) * maskB)
