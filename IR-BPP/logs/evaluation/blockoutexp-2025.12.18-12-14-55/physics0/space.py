        ...
        for rotIdx in range(rotNum):
            ...
            for X in range(self.rangeX_A - rangeX_OA + 1):
                for Y in range(self.rangeY_A - rangeY_OA + 1):
                    ...
                    # print("coorX:",coorX)
                    # print("coorY:",coorY)
                    # print("x_oh:",rangeX_OH)
                    # print("y_oh:",rangeY_OH)
                    
                    # posZ = np.max((self.heightmapC[coorX: coorX + rangeX_OH, coorY: coorY + rangeY_OH]
                    #               - heightMapB) * maskB)
                    # Skip if footprint would go out of the heightmap bounds
                    if (coorX + rangeX_OH > self.heightmapC.shape[0]) or (coorY + rangeY_OH > self.heightmapC.shape[1]):
                        continue

                    patch = self.heightmapC[coorX: coorX + rangeX_OH, coorY: coorY + rangeY_OH]
                    posZ = np.max((patch - heightMapB) * maskB)
