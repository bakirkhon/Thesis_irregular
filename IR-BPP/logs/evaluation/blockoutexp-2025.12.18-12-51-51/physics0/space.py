        ...
        for rotIdx in range(rotNum):
            ...
            for X in range(self.rangeX_A - rangeX_OA + 1):
                for Y in range(self.rangeY_A - rangeY_OA + 1):
                    ...
                    #               - heightMapB) * maskB)
                    hC = self.heightmapC[coorX: coorX + rangeX_OH,
                     coorY: coorY + rangeY_OH]

                    # 🔒 Align shapes safely
                    min_x = min(hC.shape[0], heightMapB.shape[0])
                    min_y = min(hC.shape[1], heightMapB.shape[1])

                    hC = hC[:min_x, :min_y]
                    hB = heightMapB[:min_x, :min_y]
                    mB = maskB[:min_x, :min_y]

                    posZ = np.max((hC - hB) * mB)
