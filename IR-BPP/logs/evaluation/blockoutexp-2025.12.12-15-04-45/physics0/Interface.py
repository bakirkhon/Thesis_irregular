...
class Interface:
    def __init__(self, bin=[10, 10, 5],
        ...
        self.simulationScale = simulationScale if simulationScale else 1
        ...
        if self.visual:
            ...
            p.setPhysicsEngineParameter(fixedTimeStep=1 / 240, numSubSteps=4)
    ...
    def reset(self, new_bin=None):
        ...
        print(f"🔄 Rebuilding bin with size {self.bin}")
        self.addBox(self.bin, [1, 1, 1], [0, 0, 0])
    ...
    def addBox(self, bin, scale, shift):
        ...
        for index, side in enumerate(box):
            ...
            if not os.path.exists(boxPath):
                side.export(boxPath)
            if index == 0:
                wall_alpha = 1.0  # bottom plate solid
            else:
                wall_alpha = 0.2  # walls transparent

            if self.visual:
                visual_shape_id = p.createVisualShape(
                    shapeType=p.GEOM_MESH,
                    fileName=boxPath,
                    rgbaColor=[0.8, 0.8, 0.9, wall_alpha],
    ...
    # Function to check overlap in 2D
    ...
    def addObject(self, name, targetFLB=[0.0, 0.0, 0.0], rotation=[0.0, 0.0, 0.0], scale=None,
                  density=1.0, linearDamping=0.1, angularDamping=0.1, path=None, color=None):
        if scale is None:
        ...
        if self.visual and color is not None:
            ...
            visual_shape_id = p.createVisualShape(shapeType=p.GEOM_MESH,
                                                  ...
                                                  meshScale=scale * self.simulationScale,
                                                  rgbaColor=color,
                                                  specularColor=[0.5, 0.5, 0.5])  # soft reflections
    ...
    def simulateToQuasistatic(self, givenId=None, linearTol=0.001, angularTol=0.001, batch=1.0, dt=0.01,
                              maxBatch=5):
        end = False
    ...
    def simulateToQuasistaticRecord(
        self,
        givenId=None,
        linearTol=0.001,
        angularTol=0.001,
    ...
    def reset_trimesh_Position_And_Orientation(self, id, targetFLB, targetOrientation=None):
        ...
        positionFLB, orientationT, positionBase = self.get_trimesh_Position_And_Orientation(
            id,
            inner=True,
            getPosBase=True
        )
