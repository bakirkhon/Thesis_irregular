...
class FixedListCreator(ItemCreator):
    """
    Loads a trajectory (like LoadItemCreator), 
    sorts it by volume/footprint, 
    ...
    def __init__(self, data_name, infoDict, lists_by_bin):
        ...
        print("FixedListCreator: loading", data_name)
        self.item_trajs = torch.load(self.data_name)
        self.traj_nums = len(self.item_trajs)
    ...
    def reset(self, bin_idx, traj_index):
        """
        Called by PackingGame at the start of each bin.
        This method:
        1. Loads the next trajectory
        2. Sorts it by score
        3. Selects the correct subset for this bin using lists_by_bin
        """

        self.item_list.clear()
        self.index = 0

        # Load next trajectory (loop around)
    ...
    # -----------------------
    # PREVIEW (hierarchical RL uses this)
    # -----------------------
    def preview(self, length):
        """
