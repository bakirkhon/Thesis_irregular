        ...
        for rotIdx in range(rotNum):
            ...
            for X in range(self.rangeX_A - rangeX_OA + 1):
                for Y in range(self.rangeY_A - rangeY_OA + 1):
                    ...
                    #               - heightMapB) * maskB)
                    # --- SAFE slicing to avoid boundary overflow ---
                    x_end = min(coorX + rangeX_OH, self.heightmapC.shape[0])
                    y_end = min(coorY + rangeY_OH, self.heightmapC.shape[1])

                    hm_slice = self.heightmapC[coorX:x_end, coorY:y_end]

                    # Match shapes
                    hB = heightMapB[:hm_slice.shape[0], :hm_slice.shape[1]]
                    mB = maskB[:hm_slice.shape[0], :hm_slice.shape[1]]

                    posZ = np.max((hm_slice - hB) * mB)
