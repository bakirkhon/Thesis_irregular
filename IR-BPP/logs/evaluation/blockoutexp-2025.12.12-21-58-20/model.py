...
class DQNBPP(nn.Module):
  ...
  def embed_heightmap_and_sampled_point_cloud(self, x):
    ...
    #   map_feature = self.heightEncoder(heightMap).reshape((batchSize, -1))
        # Calculate MapLength dynamically from the actual heightMap size
        
      heightmap_total_size = heightMap.shape[1]
      MapLength = int(np.sqrt(heightmap_total_size))
        
        # Reshape to proper 2D heightmap
      heightMap = heightMap.reshape((batchSize, 1, MapLength, MapLength))
        
      # Pass through CNN encoder
      map_feature_raw = self.heightEncoder(heightMap)
      if hasattr(self, 'adaptive_pool'):
        map_feature_pooled = self.adaptive_pool(map_feature_raw)
        map_feature = map_feature_pooled.reshape((batchSize, -1))
