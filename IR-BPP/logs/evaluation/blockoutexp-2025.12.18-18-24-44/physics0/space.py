        ...
        for rotIdx in range(rotNum):
            ...
            for X in range(self.rangeX_A - rangeX_OA + 1):
                for Y in range(self.rangeY_A - rangeY_OA + 1):
                    ...
                    # 1️⃣ grid safety
                    if (coorX + rangeX_OH > self.rangeX_C) or (coorY + rangeY_OH > self.rangeY_C):
                        continue

                    # 2️⃣ continuous-space safety (THIS prevents wall intersection)
                    if (
                        (coorX * self.resolutionH + boundingSize[0] > self.bin_dimension[0] - 1e-3) or
                        (coorY * self.resolutionH + boundingSize[1] > self.bin_dimension[1] - 1e-3)
                    ):
                        continue
                    ...
                    posZ = np.max((hC - heightMapB) * maskB)

                    if np.round(posZ + boundingSize[2] - self.bin_dimension[2], 6) <= 0:
                        naiveMask[rotIdx, X, Y] = 1
