        ...
        for rotIdx in range(rotNum):
            ...
            for X in range(self.rangeX_A - rangeX_OA + 1):
                for Y in range(self.rangeY_A - rangeY_OA + 1):
                    ...
                    # FIX: Ensure we don't exceed heightmapC boundaries
                    endX = min(coorX + rangeX_OH, self.rangeX_C)
                    endY = min(coorY + rangeY_OH, self.rangeY_C)
                    
                    # Get the actual slice from heightmapC
                    heightmap_slice = self.heightmapC[coorX:endX, coorY:endY]
                    
                    # Get the corresponding slice from heightMapB and maskB
                    sliceX = endX - coorX
                    sliceY = endY - coorY
                    heightMapB_slice = heightMapB[:sliceX, :sliceY]
                    maskB_slice = maskB[:sliceX, :sliceY]
                    
                    # Calculate posZ with matching shapes
                    posZ = np.max((heightmap_slice - heightMapB_slice) * maskB_slice)
