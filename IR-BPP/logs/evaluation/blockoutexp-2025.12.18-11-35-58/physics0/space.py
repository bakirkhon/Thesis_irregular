        ...
        for rotIdx in range(rotNum):
            ...
            for X in range(self.rangeX_A - rangeX_OA + 1):
                for Y in range(self.rangeY_A - rangeY_OA + 1):
                    ...
                    #               - heightMapB) * maskB)
                    # --- Clamp slice to heightmap bounds (robust for any bin size) ---
                    x0, y0 = coorX, coorY
                    x1 = min(coorX + rangeX_OH, self.heightmapC.shape[0])
                    y1 = min(coorY + rangeY_OH, self.heightmapC.shape[1])

                    patch = self.heightmapC[x0:x1, y0:y1]

                    # If patch got clipped at border, clip item maps to match
                    hx = x1 - x0
                    hy = y1 - y0
                    hb = heightMapB[:hx, :hy]
                    mb = maskB[:hx, :hy]

                    posZ = np.max((patch - hb) * mb)
